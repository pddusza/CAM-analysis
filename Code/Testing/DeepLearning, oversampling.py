...
import torchmetrics
import random
from MultilabelBalancedRandomSampler import MultilabelBalancedRandomSampler
...
for model in model_list:
    ...
    train_sampled=np.array(train_csv.drop(['FileName','TAG'], axis=1))
    # Create a MultilabelBalancedRandomSampler to perform oversampling during training
    train_sampler = MultilabelBalancedRandomSampler(
        train_sampled, range(len(train_sampled)), class_choice="least_sampled"
    )
    # Validation dataset
    ...
    for epoch in range(epochs):
        ...
        if valid_f1>valid_f1_temp:
            ...
            torch.save(model.state_dict(), f'model, oversampled good mesophagus highest f1 {valid_f1} 08-08.pth')
    ...
    torch.save(
        model.state_dict()
    , 'end model, oversampled good mesophagus 08-08.pth')
